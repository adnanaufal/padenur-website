          ...
          <div className="hidden md:flex items-center space-x-1">
            {navLinks.map((link) => (
              <Link
                key={link.href}
                href={link.href}
                className="text-gray-300 hover:text-gold px-3 py-2 rounded-md text-sm font-medium transition-colors duration-200"
              >
                {link.label}
              </Link>
            ))}
            <a
              ...
              className="ml-3 inline-flex items-center gap-1.5 bg-gold/10 text-gold hover:bg-gold/20 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 border border-gold/20"
            >
          ...
          <div className="px-4 py-3 space-y-1">
            {navLinks.map((link) => (
              <Link
                ...
                className="block text-gray-300 hover:text-gold hover:bg-white/5 px-3 py-2.5 rounded-md text-sm font-medium transition-colors"
              >
                {link.label}
              </Link>
            ))}
